...
import { formatCentsILS, formatDateDDMMYYYY, HEBREW_MONTH_NAMES } from "@/lib/format";
...
export default async function CalendarPage({ searchParams }: { searchParams: Promise<SearchValues> }): Promise<React.JSX.Element> {
  ...
  return <div className="flex min-w-0 flex-col gap-4"><h1 className="text-2xl font-bold">תאריכון</h1><TotalsStrip totals={summarize(rows)} />{rows.some(row=>row.currency_review_required)&&<p role="alert">יש תנועות ישנות עם מטבע שדורש בדיקה. הן מופיעות ברשימת היום ואינן נכללות בסכומים עד לתיקון.</p>}<p className="text-sm">{docs.count ?? 0} מסמכים הועלו החודש. הסכומים לפי תאריך התנועות; יתרת מע״מ שלילית היא יתרת זכות.</p>
    ...
    <div className="grid grid-cols-7 overflow-hidden rounded-lg border border-border" dir="rtl">{["ראשון", "שני", "שלישי", "רביעי", "חמישי", "שישי", "שבת"].map(day => <div key={day} className="bg-foreground/5 py-2 text-center text-xs sm:text-sm">{day}</div>)}
      {Array.from({ length: totalCells }, (_, cell) => { const day = cell - leading + 1; if (day < 1 || day > count) return <div key={cell} className="min-h-20 border-t border-border bg-foreground/5" />;
        ...
        return <Link key={cell} href={`/calendar?month=${month}&day=${date}#day-transactions`} scroll={false} aria-label={`${formatDateDDMMYYYY(date)}, ${entries.length} תנועות${income ? `, הכנסות ${formatCentsILS(totals.income)}` : ""}${expense ? `, הוצאות ${formatCentsILS(totals.expense)}` : ""}`} aria-current={selected === date ? "date" : undefined} className={`min-h-24 min-w-0 border-t border-border p-1 text-center focus-visible:outline-2 focus-visible:outline-primary sm:p-2 ${selected === date ? "bg-primary/15 ring-2 ring-inset ring-primary" : entries.length ? "bg-primary/5" : ""}`}><span className={`inline-block text-sm ${date === today ? "font-bold underline" : ""}`}>{day}</span><div className="mt-2 text-[9px] leading-4 sm:text-xs"><span className="block truncate text-income" title={formatCentsILS(totals.income)}>{income ? formatCentsILS(totals.income) : ""}</span><span className="block truncate text-expense" title={formatCentsILS(totals.expense)}>{expense ? formatCentsILS(totals.expense) : ""}</span></div></Link>;
      })}
    </div>
