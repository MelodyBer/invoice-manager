...
import { metricCookieName, parseMetricSelection } from "@/lib/dashboard/metrics";
...
export default async function DashboardPage({ searchParams }: { searchParams: Promise<SearchValues> }): Promise<React.JSX.Element> {
    ...
    try {
        ...
        const selectedMetrics = parseMetricSelection((await cookies()).get(metricCookieName(userId))?.value);
        return <DashboardView data={data} selectedMetrics={selectedMetrics} />;
    } catch (error: unknown) {
