...
export function MetricCards({ totals, comparison, initialSelection }: { totals: Summary; comparison: Comparison; initialSelection: DashboardMetricId[] }): React.JSX.Element {
    ...
    return <section aria-label="מדדי העסק" className="space-y-4">
        ...
        {selected.length === 0 ? <p className="rounded-xl bg-primary/5 p-4 text-sm">לא נבחרו כרטיסים. אפשר להוסיף אותם דרך „בחירת מדדים לתצוגה”.</p> : <div className="grid min-w-0 grid-cols-2 gap-3 sm:gap-4 xl:grid-cols-4">{DASHBOARD_METRICS.filter(metric => selected.includes(metric.id)).map(metric => {
            ...
            return <section key={metric.id} className={`min-w-0 rounded-2xl border border-border bg-background p-3 sm:p-6 ${metric.featured ? "border-primary/40 bg-primary/5 ring-1 ring-primary/15" : ""}`}><h2 className="text-xs font-medium text-foreground/70 sm:text-sm">{refund ? "החזר מע״מ צפוי" : metric.label}</h2><p className={`mt-2 break-words text-xl font-bold tracking-tight sm:mt-3 sm:text-3xl ${tone}`} dir="ltr">{formatCentsILS(refund ? Math.abs(totals.vatDue) : totals[metric.id])}</p>{metric.id === "expenseTotal" && <p className="mt-2 text-xs text-foreground/60">סכום מסמכי ההוצאה, כולל מע״מ כשיש. אינו בהכרח סכום ששולם בפועל.</p>}<Change value={comparison[metric.id]} /></section>;
        })}</div>}
    </section>;
}
