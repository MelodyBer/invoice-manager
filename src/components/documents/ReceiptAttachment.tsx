...
export function ReceiptAttachment({documentId,values}:{documentId:string;values:TransactionFormValues}):React.JSX.Element{
 ...
 return <section id="attach-receipt" className="rounded-xl border-2 border-primary p-4"><h2 className="font-bold">חיפוש ידני בין חשבוניות שאושרו</h2><p className="my-2 text-sm">הרשימה מציגה חשבוניות אחרונות, ולא התאמות אוטומטיות לקבלה. חפשי לפי שם או מספר ובדקי את פרטי החשבונית לפני הצירוף. הצירוף אינו יוצר תנועה נוספת.</p><div className="flex flex-wrap items-end gap-2"><Input id="invoice-search" label="חיפוש לפי שם או מספר חשבונית" value={term} onChange={event=>setTerm(event.target.value)} /><Button variant="secondary" isLoading={loading} onClick={()=>void search()}>חפש חשבונית</Button></div>
