...
export function MergeApprovedDocuments({transaction,onMerged}:{transaction:TransactionRow;onMerged?:()=>void}):React.JSX.Element|null{
 ...
 {comparison&&<div className="mt-4"><div className="grid min-w-0 gap-4 xl:grid-cols-2">{([["חשבונית המס שתישאר בתנועות",comparison.invoice],["הקבלה שתצורף לחשבונית",comparison.receipt]] as const).map(([label,detail])=><section key={detail.transaction.id} className="min-w-0 rounded border border-border p-3"><h3 className="font-bold">{label}</h3><p className="break-words">{detail.transaction.counterparty_name} · {detail.transaction.doc_number??"ללא מספר"}</p><MoneySummary value={detail.transaction} date={detail.transaction.doc_date}/>{detail.documents.map(document=><DocumentViewer key={document.id} storagePath={document.storage_path} mimeType={document.mime_type}/>)}</section>)}</div>
