...
export function TransactionDrawer({ id, onClose }: { id: string; onClose: () => void }): React.JSX.Element {
  ...
  return <dialog ref={ref} aria-labelledby="transaction-detail-title" onCancel={event => { if (busy) event.preventDefault(); else onClose(); }} className="fixed inset-y-0 right-0 left-auto m-0 h-dvh max-h-none w-full max-w-2xl overflow-y-auto border-s border-border bg-background p-4 text-foreground shadow-xl backdrop:bg-black/40 sm:p-6">
    ...
    {detail && <>
      ...
      {confirmDelete ? <section role="alert" className="my-4 rounded border border-expense p-4"><p>למחוק את התנועה? {detail.document ? "המסמך המקורי יימחק גם הוא. לא ניתן לבטל את המחיקה." : "לא ניתן לבטל את המחיקה."}</p><div className="mt-3 flex gap-2"><Button variant="secondary" disabled={busy} onClick={() => setConfirmDelete(false)}>בטל</Button><Button className="bg-expense" isLoading={busy} onClick={() => void remove()}>מחק לצמיתות</Button></div></section> : <div className="my-5 flex gap-2">{editing ? <><Button isLoading={busy} onClick={() => void save()}>שמירת שינויים</Button><Button variant="secondary" disabled={busy} onClick={() => { setEditing(false); setDuplicateId(null); }}>ביטול עריכה</Button></> : <Button onClick={() => setEditing(true)}>עריכה</Button>}<Button variant="secondary" disabled={busy} onClick={() => setConfirmDelete(true)}>מחיקה</Button></div>}
      {detail.document && <section className="mt-6"><h3 className="mb-3 font-semibold">המסמך המקורי</h3><DocumentViewer key={detail.document.id} storagePath={detail.document.storage_path} mimeType={detail.document.mime_type} /></section>}
    </>}
  </dialog>;
}
