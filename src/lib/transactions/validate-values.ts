import { toCents, fromCents, addCents } from "@/lib/calc";
...
export function validateTransactionValues(values: TransactionFormValues): string | null {
  ...
  if (amounts.some(value => !value.trim() || !Number.isFinite(Number(value)) || Number(value) < 0 || Number(value) >= 1e10 || fromCents(toCents(Number(value))) !== Number(value))) return "יש להזין סכומים תקינים, עד שתי ספרות אחרי הנקודה.";
  if (Number(values.amountTotal) <= 0) return "הסכום הכולל חייב להיות גדול מאפס.";
  if (Math.abs(addCents(toCents(Number(values.amountBeforeVat)), toCents(Number(values.vatAmount)), -toCents(Number(values.amountTotal)))) > 1) return "הסכומים אינם תואמים. בדקי את הסכום הכולל והמע״מ.";
