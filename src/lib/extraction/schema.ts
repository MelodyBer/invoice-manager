...
- המסמכים יכולים להיות בעברית או באנגלית. זהי את מטבע הסכומים במסמך: ILS לשקל או USD לדולר אמריקאי. אל תחליפי דולר בשקל ואל תבצעי המרת מטבע. הסכומים מוחזרים במטבע המקורי. אם מופיעים כמה מטבעות, צייני זאת בהערה ובחרי את המטבע של סך התשלום הראשי.
...
- במסמך דולרי או מספק זר אין להניח מע״מ ישראלי של 18%. אם אין פירוט מס, החזירי vat_amount=0, vat_rate=0 ו-amount_before_vat=amount_total והוסיפי הערה שהמס דורש בדיקה.
- במסמך שקלי ישראלי, אם מופיע רק הסכום הכולל בלי פירוט מע״מ ואין בו ציון פטור או ללא מע״מ, יש לחשב לאחור לפי שיעור מע״מ של 18%: amount_before_vat = amount_total חלקי 1.18, ו-vat_amount = amount_total פחות amount_before_vat.
